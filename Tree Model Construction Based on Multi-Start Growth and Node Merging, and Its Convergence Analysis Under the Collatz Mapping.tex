...
\begin{document}
	...
	This appendix gives a formal graph-theoretic definition of the tree models (odd tree and complete tree).  
	The goal is to abstract all structures in the main text—nodes, growth edges, branch edges, and connection edges—into a directed graph,  
	so that all subsequent theorems can be stated and verified in standard mathematical terms.
	...
	The odd tree contains all positive odd integers, and the complete tree contains all positive integers.
	
	\subsection*{C.2 Definition of the Graph Structure}
	
	\begin{definition}[Directed Tree Structure]
		...
		\begin{enumerate}[label=(\arabic*)]
			\item $V$ is the set of nodes, a subset of $\mathbb{N}_{>0}$; in the odd tree $V=\mathbb{N}_{\mathrm{odd}}$, in the complete tree $V=\mathbb{N}_{>0}$.
			
			\item $E \subseteq V\times V$ is the set of directed edges, denoted $u\to v$.  
			Edge directions are determined by construction rules (growth, branch, connection) and always point to the “newly generated node.”
			
			\item $r\in V$ is the root; here $r=1$.
			
			\item $\pi: V\setminus\{r\}\to V$ is the parent map, satisfying
			\[
		...
		Additionally, $(V,E)$ contains no directed cycles except for the special self-loop around the root.
	\end{definition}
	
	The root node $1$ has no parent, and all other nodes have a unique parent; thus $\mathcal{T}$ is a rooted directed tree.
	
	\subsection*{C.3 Growth, Branch, and Connection Edges}
	
	Edges in the tree are defined by three types of generation rules:
	
	\begin{definition}[Growth Edge]
		\label{def:growth-edge}
		If $u\in \mathbb{N}_{\mathrm{odd}}$ and $v=u\cdot 2^k$ is an even node generated by the growth rule ($k\ge1$), define
		\[
		u\to v \in E.
		\]
		This produces the odd-to-even growth chain.
	\end{definition}
	
	\begin{definition}[Branch Edge]
		\label{def:branch-edge}
		If $v$ (odd or even) satisfies $(v-1)/3\in \mathbb{N}_{>0}$ and is odd, then the “reverse” odd node
		\[
		u = \frac{v-1}{3}
		\]
		is generated, and
		...
		u\to v \in E.
		\]
		In the odd tree, only odd branches are kept; in the complete tree, $v$ can be even.
	\end{definition}
	
	\begin{definition}[Connection Edge (Class Merging)]
		\label{def:connect-edge}
		If a branch node $u$ coincides with a starting or growth node $u'$, treat $u$ and $u'$ as the same node,  
		and merge all substructures generated by them.  
		No additional edges are added; only the parent map $\pi$ is kept consistent and unique.
	\end{definition}
	
	Connection edges do not add actual edges; they merge node identities so that different sources converge to the same node.
	
	\subsection*{C.4 Reverse Edges and the Inverse Graph}
	
	To describe the “reverse regression” process (corresponding to the Collatz descent), define reverse edges:
	
	\begin{definition}[Reverse Edges and Inverse Graph]
		Define the reverse edge set
		\[
		E^{-1} = \{(v,u) \in V\times V : u\to v \in E\}.
		\]
		The inverse graph is
		\[
		\mathcal{T}^{-1} = (V,E^{-1}).
		\]
		Edges indicate backward regression: if $v$ points to $u$ in $E^{-1}$, then $u$ is the parent $\pi(v)$.
	\end{definition}
	
	Specifically:
	
	\begin{itemize}
		\item If $v$ is even, there is a unique reverse edge $v\to v/2$;
		\item If $v$ is odd and $v\equiv 1 \pmod 3$, there is a unique reverse edge $v\to (v-1)/3$;
		\item The root node $1$ has no reverse edges.
	\end{itemize}
	
	\subsection*{C.5 Depth and Layer}
	
	\begin{definition}[Depth]
		Define the depth function $\mathrm{depth}:V\to\mathbb{Z}_{\ge0}$ as the minimal number of steps along the parent map from the root to reach a node:
		\[
	...
	With this, the graph is fully formalized as a directed acyclic tree.
	
	\subsection*{C.6 Correctness Requirements of the Graph Structure}
	
	Construction rules must satisfy:
	
	\begin{enumerate}[label=(\arabic*)]
		\item Every $v\neq 1$ has a unique parent $\pi(v)$;
		\item No non-trivial directed cycles exist;
		\item Node identity merging (connection rule) does not create multiple parents;
		\item In the complete tree model: the odd subgraph is an induced subgraph, and even nodes do not violate tree structure.
	\end{enumerate}
	
	This ensures the tree model is mathematically well-defined, verifiable, and serves as a unified foundation for all theorems in the main text.
