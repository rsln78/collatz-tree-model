...
%-------------------- 摘要 --------------------
	...
	\]
	于是 $x=x'$ 且 $n=n'$。因此当 $(n,x)\neq(n',x')$（即 $x\neq x'$ 或 $n\neq n'$）时，等式无解。换言之：\(\mathbf{不同\ 层\ 的\ B_1\ 序列\ 互不相交}\)。
	...
	\]
	因此 $x=x'$ 且 $m=m'$。故 \(\mathbf{不同\ 层\ 的\ B_5\ 序列\ 互不相交}\)。
	...
	2x = 2x'-1.
	\]
	但左端为偶数，右端为奇数，这在整数中不可能成立。因此等式 (*) 在任何整数 $x,x'$ 下都无解。换言之：\(\mathbf{任何\ B_1\ 项均不可能等于任意\ B_5\ 项}\)。
	...
	\subsection*{C.3 生长边、分支边与连接边的形式化定义}
	...
	\begin{definition}[分支边]
		\label{def:branch-edge}
		若节点 $v$（奇或偶）满足 $(v-1)/3\in\mathbb{N}_{>0}$ 且该值为奇数，则产生“逆向”奇数节点
		\[
		u=\frac{v-1}{3},
		\]
		并定义边
		...
		u\to v \in E.
		\]
		在奇数树中仅保留奇数分支；在完整树中允许 $v$ 为偶数。
	\end{definition}
	
	\begin{definition}[连接边（类间合并）]
		\label{def:connect-edge}
		若某分支节点 $u$ 与某起点节点或生长节点 $u'$ 数值相同，则视 $u$ 与 $u'$ 为同一节点，并将其各自生成的全子结构合并。此时不新增额外边，仅保证父映射 $\pi$ 的一致性与唯一性不被破坏。
	\end{definition}
	...
	- 若 $v$ 为偶数，则存在唯一逆向边 $v\to v/2$；
	- 若 $v$ 为奇数且 $v\equiv 1\!\!\pmod 3$，则存在唯一逆向边 $v\to (v-1)/3$；
	- 根节点 $1$ 无逆向边。
